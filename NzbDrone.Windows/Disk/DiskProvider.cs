...
namespace NzbDrone.Windows.Disk
{
    public class DiskProvider : DiskProviderBase
    {
        ...
        public override bool TryCreateSymbolicLink(string source, string destination)
        {
            try
            {
                return CreateSymbolicLink(destination, source, 0);
            }
            catch (Exception ex)
            {
                Logger.Debug(ex, string.Format("Symlink '{0}' to '{1}' failed.", source, destination));
                return false;
            }
        }
